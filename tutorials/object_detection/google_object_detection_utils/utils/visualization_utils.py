...
def visualize_boxes_and_labels_on_image_array_2(
  ...
  for i in range(min(max_boxes_to_draw, boxes.shape[0])):
    # if scores is None or scores[i] > min_score_thresh:
    if np.max(classes[i]) > min_score_thresh:
      box = tuple(boxes[i].tolist())
      if instance_masks is not None:
        box_to_instance_masks_map[box] = instance_masks[i]
      if instance_boundaries is not None:
      ...
      if scores is None:
        ...
        if not skip_labels:
          if not agnostic_mode:
            # NEW
            ######################################
            this_classes = classes[i]
            for j in np.argsort(this_classes)[::-1]:
              # print(this_classes[i])
              if this_classes[j] < min_score_thresh:
                break
              else:
                class_str = category_index[j + 1]['name'] if j + 1 in category_index.keys() else 'N/A'
                display_str += '{}: {}% '.format(class_str, int(100*this_classes[j]))
            
        #     classes_sort = np.argsort(classes)
        #     if classes[i] in category_index.keys():
        #       class_name = category_index[classes[i]]['name']
        #     else:
        #       class_name = 'N/A'
        ...
        if agnostic_mode:
          ...
          box_to_color_map[box] = STANDARD_COLORS[
              (np.argmax(classes[i]) + 1) % len(STANDARD_COLORS)]
